...
def capabilities(config):
    ...
    return {'search':kind!='url','pagination':kind!='url',
            'pagination_mode': 'native' if native_pagination(config) else 'prefix',
            'max_results':None if native_pagination(config) else MAX_RESULTS,
            'search_rankings':['default']+(['views','recent','trending'] if dm else ['views'] if yt else []),
            'duration':dm,'date':dm,
...
def search_config(config,ranking,duration,days):
    ...
    if caps['duration']:
        parts=urlsplit(config['search_url']);params=dict(parse_qsl(parts.query))
        if ranking!='default':params['sort']={'views':'visited','recent':'recent','trending':'trending'}[ranking]
